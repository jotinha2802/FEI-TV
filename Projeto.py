...
while True:
    ...
    if menu == 2:
        if usuario_logado != "":
            print(f"VOCÊ JÁ ESTÁ LOGADO COMO: {usuario_logado}")
            print("DESEJA TROCAR DE CONTA?\n1. SIM\n2. NÃO")
            trocar = input("ESCOLHA UMA OPÇÃO: ")

            if trocar == "2":
                print("CONTINUANDO COM A CONTA ATUAL...")
            
            elif trocar == "1":
                usuario_logado = ""
                print("CONTA DESCONECTADA! FAÇA O NOVO LOGIN.")
                ...
                for Email, Senha in lista:
                    if Email_Login == Email:
                        ...
                        if Senha_Login == Senha:
                            ...
                            nome_arquivo = f"Favoritos_{usuario_logado}.txt"
                            arquivo_fav = open(nome_arquivo, 'a', encoding='utf-8')
                            arquivo_fav.close()
    ...
    if menu == 5:
        if usuario_logado == "":
            ...
            favoritos = int(input("O QUE VOCÊ DESEJA FAZER?\n"))
            print("=" * 50)

            if favoritos == 1:
                arquivo_fav = open(nome_arquivo, 'r', encoding='utf-8')
